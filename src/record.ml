...
let callbacks t =
  Runtime_events.Callbacks.create ()
    ...
    ~lifecycle:(fun ring_id ts event _data ->
        ...
        match event with
        | EV_DOMAIN_SPAWN -> Write.instant_event t.fxt ~thread ~ts ~name:"domain-spawn" ~category:"ocaml"
        | EV_DOMAIN_TERMINATE -> Write.instant_event t.fxt ~thread ~ts ~name:"domain-terminate" ~category:"ocaml"
        | _ -> ()
      )
