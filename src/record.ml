...
let callbacks t =
  Runtime_events.Callbacks.create ()
    ...
    ~lifecycle:(fun ring_id ts event data ->
        ...
        | EV_DOMAIN_SPAWN ->
          let args =
            match data with
            | None -> []
            | Some tid -> ["tid", `Int64 (Int64.of_int tid)]    (* XXX: requires compiler patch *)
          in
          Write.instant_event t.fxt ~thread ~ts ~name:"domain-spawn" ~category:"ocaml" ~args
        | EV_DOMAIN_TERMINATE ->
